...
@available(iOS 13, *)
typealias PassportReader = EMRTDTagReader

@available(iOS 13, *)
public class EMRTDTagReader : NSObject {
    ...
    public func readPassport( mrzKey : String, tags: [DataGroupId] = [], skipSecureElements :Bool = true, skipCA : Bool = false, skipPACE: Bool = true, customDisplayMessage: ((NFCViewDisplayMessage) -> String?)? = nil, completed: @escaping (NFCPassportModel?, NFCPassportReaderError?)->()) {
        readEMRTDTag(accessKey: .mrz(mrzKey),
                     tags: tags,
    ...
    public func readEMRTDTag( accessKey : PACEAccessKey, tags: [DataGroupId] = [], skipSecureElements :Bool = true, skipCA : Bool = false, skipPACE: Bool = false, customDisplayMessage: ((NFCViewDisplayMessage) -> String?)? = nil, completed: @escaping (NFCPassportModel?, NFCPassportReaderError?)->()) {
...
@available(iOS 13, *)
extension EMRTDTagReader : NFCTagReaderSessionDelegate {
...
//                self?.invalidateSession(errorMessage: displayMessage, error: error)
    ...
    func handleBAC( completed: @escaping (NFCPassportReaderError?)->()) {
        ...
        case .none:
            ...
            completed(.AuthenticationFailed)
        case .mrz( let mrzKey ):
            self.bacHandler = BACHandler( tagReader: tagReader )
    ...
    func handlePACE( cardAccess:CardAccess, completed: @escaping (NFCPassportReaderError?)->()) {
        ...
        do {
            ...
            paceHandler?.doPACE( accessKey: accessKey ) { paceSucceeded in
                if paceSucceeded {
                    completed(nil)
                } else {
                    self.paceHandler = nil
